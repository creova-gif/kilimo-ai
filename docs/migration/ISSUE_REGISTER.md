...
| KIL-002 | P1 | Mobile-money shows a false "sent" success | `app/mobile-money.tsx:162-172` | Code read | No success message without a backend transaction | No provider integrated; seeded transactions | Open |
| KIL-003 | P1 | Shamba tab and farm/plot screens run on hard-coded fake zones | `constants/FarmData.ts:26`, `(tabs)/fields.tsx:53,125`, `field/[id].tsx:267`, `vra-setup.tsx:41` | Code read | User's real farms/plots | No farm/plot tables or hooks; store is AsyncStorage-only | Open |
| KIL-004 | P1 | Finance, insurance, IoT, wallet-admin, peer-groups, livestock, inventory present seeded/simulated data as if real | see FEATURE_INVENTORY.md | Code read | Real data or clearly unavailable | Prototype code | Open |
...
| KIL-010 | P2 | `lib/session.ts` `signOutEverywhere` not wired | `lib/session.ts` | Grep | Profile sign-out uses offline-safe sign-out | New file, no importer | Open |
